...
4. Remote Case, some commits has been pushed to commit:

    At the same time, someone commited a change, namely commit `D`, pushed the commit to repository server. 
    If `D` is before `C`, we call it `Remote Case A`, otherwise, `Remote Case B`
...
```
Case A: Local Case A + Remote Case C

A -- B      [b1]
      \
       C    [b1]

Case B: Local Case B + Remote Case C
